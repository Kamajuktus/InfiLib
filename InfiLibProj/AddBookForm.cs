...
namespace InfiLibProj
{
    public partial class AddBookForm : Form
    {
        ...
        {
            DB db = new DB();
            MySqlCommand AddBook = new MySqlCommand("INSERT INTO `books` (`name`, `author`, `genre`, `description`, `issued`, `image`) VALUES (@name, @author, @genre, @desc, @issued, @image);", db.getConnection());

            MySqlCommand refreshIncrement = new MySqlCommand("ALTER TABLE `books` AUTO_INCREMENT=1;", db.getConnection());

            AddBook.Parameters.Add("@name", MySqlDbType.VarChar).Value = BookNameAdd.Text;
            AddBook.Parameters.Add("@author", MySqlDbType.VarChar).Value = BookAddComboBoxAuthor.Text;
            AddBook.Parameters.Add("@genre", MySqlDbType.VarChar).Value = BookAddComboBoxGenre.Text;
            AddBook.Parameters.Add("@desc", MySqlDbType.VarChar).Value = BookDescriptionBoxAdd.Text;
            AddBook.Parameters.Add("@issued", MySqlDbType.Int16).Value = 0;
            AddBook.Parameters.Add("@image", MySqlDbType.VarChar).Value = AddPictureBox.ImageLocation;
            ...
            for(int i = 0; i < Int16.Parse(AddBookQuantity.Text); i++)
            {
